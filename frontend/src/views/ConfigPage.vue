...
async function loadDownloadSettings(){try{const r=await getSettings();if(r?.status===200&&r.data){downloadDir.value=r.data.download_dir||'downloads';saveLocal.value=r.data.save_local!==false;browserDownload.value=r.data.browser_download===true}}catch(e){}}
async function saveDownloadSettings(){savingDownload.value=true;try{await saveSettings({download_dir:downloadDir.value,save_local:saveLocal.value,browser_download:browserDownload.value});window.__snackbar?.('已保存','success')}catch(e){window.__snackbar?.('保存失败','error')}finally{savingDownload.value=false}}
