...
- [x] **多模型特征权重聚合引擎 (后端)**：建立 `weight_config.json` 矩阵，将 4 大时段与跨模型指标（声学/流派/意境/分离轨道）进行融合加权排序
- [x] **"上帝模式"独立控制台面板 (前端)**：独立于雷达图，使用类音频调音台（Slider）的 UI，提供全天候 4 个时段的高级推送比重配置页面

#### 📊 智能画像与可视化看板 (待启动)

- [ ] **用户喜欢类型雷达图可视化**：聚合历史听歌数据，渲染多模型特征交叉的用户长周期“灵魂画像”
- [ ] **歌单解析雷达图生成**：一键导入外部歌单（如网易云歌单），实时解析该歌单的整体听感特征分布
- [ ] **猜你喜欢推荐算法**：基于 Librosa/PANNs/Ollama/Demucs 多维特征向量的相似度（Cosine Similarity），实现精准私有化推荐
- [ ] **猜你不喜欢避坑系统**：黑名单机制，根据不喜欢的音乐特征主动克隆排除类似曲风
