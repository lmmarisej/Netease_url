...
- [ ] **Aero-Material 四轨 Tab 联动面板 (前端)**：将播放、谱图、调音台、历史整合至同一菜单页下，切换采用果味拟态高通透毛玻璃视觉
- [ ] **智能发现与隐式行为埋点 (前端/后端)**：集成网易云热榜与手动歌单拉取源；前端无感监测播放时长，未满 20% 切换自动上报 `is_skipped` 日志闭环
- [ ] **历史追踪与有效聆听轴 (前端)**：实现瀑布流式听歌历史看板，可视化每首歌的实际聆听比例，为后续优化相似度算法提供隐式反馈数据
