...
- [ ] 添加用户注册功能
- [x] 📡 API 文档 Postman 化改造 — 接口调试页面左右布局、Composable 拆分、scoped 样式子组件化
- [x] ~~可对当前账号喜欢的音乐镜像操作~~
- [x] 同步功能，开启歌单完全匹配功能，开启后歌单同步删除歌曲，本地也删除
- [x] 🎵 音乐智能推荐 — Librosa 基础声学维度（7 维评分，music_processor.py）
- [x] 音乐智能推荐 — 引入 PANNs 模型，实现全自动百种标签生成（音色/流派/乐器）
- [ ] 音乐智能推荐 — 引入 Ollama + LLM，抽取歌词高级意境主题（深夜/国风/热血）
- [ ] 音乐智能推荐 — 引入 Demucs 声源分离，拆解人声与贝斯实现细节口味匹配
- [ ] 音乐智能推荐 — 用户喜欢类型雷达图可视化
- [ ] 音乐智能推荐 — 歌单解析雷达图生成
- [ ] 音乐智能推荐 — 猜你喜欢（基于多维特征相似度推荐）
- [ ] 音乐智能推荐 — 猜你不喜欢（歌单不喜欢音乐自动排除）
